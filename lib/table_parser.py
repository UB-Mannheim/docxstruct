...
class Table(object):
    ...
    def analyse_structure(self, content_lines, feature_lines, template="datatable"):
        ...
        if template in ["datatable", "datatable_balance"]:
            ...
            if wordratio > 0.5:
                if widx >= 1:
                    ...
                    if not self._vali_date(features, content):
                        if self.info.start is True and self.info.regex.lastidxnumber.search(content["text"][:]) \
    ...
    def _extract_bboxlevel(self, entry) -> bool:
        ...
        for idx in range(0, self.structure["gapidx"][self.info.lidx] + 1):
            self.content[self.structure["btype"][self.info.lidx]][0][self.info.row].append(
                ''.join([i for i in ''.join(entry['words'][idx]["text"]) if i.isdigit() or i == " "]))

        self.content[self.structure["btype"][self.info.lidx]][0][self.info.row] = " ".join(
            self.content[self.structure["btype"][self.info.lidx]][0][self.info.row]).strip()
        ...
        self.content[self.structure["btype"][self.info.lidx]][1][self.info.row] = " ".join(
            self.content[self.structure["btype"][self.info.lidx]][1][self.info.row]).strip()
        return True
    ...
    def _extract_textlevel(self, entry, features) -> bool:
        ...
        for grpidx, numbergrp in enumerate(reversed(numbers)):
            ...
            if len(numbergrp) == 3 and grpidx != len(numbers) and count_numbers < (
                ...
                count_numbers = 0
                self.content[self.structure["btype"][self.info.lidx]][count_years][self.info.row] = (
                            numbergrp + " " + number).strip()
                number = ""
                count_years -= 1
                if count_years == 0:
                    self.content[self.structure["btype"][self.info.lidx]][count_years][self.info.row] = " ".join(
        ...
        if self.info.separator < entry["hocr_coordinates"][2]:
            try:
                ...
                if resultex.findall(numbers):
                    self.content[self.structure["btype"][self.info.lidx]][0][self.info.row] = numbers[
                                                                                              :-len(result)].strip()
                    self.content[self.structure["btype"][self.info.lidx]][1][self.info.row] = numbers[
                                                                                              -len(result):].strip()
                    return True
            except:
